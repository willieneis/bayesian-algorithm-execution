...
for i in range(n_iter):
    ...
    # Compute a mode of current posterior over output
    last_output_list = acqopt.get_last_output_list()
    acq = AcqFunction({'acq_str': 'out'})
    cluster_idx_list = acq.get_samp_cluster_idx_list(last_output_list)
    #print('cluster_idx_list:')
    #print(cluster_idx_list)
    #input('Pause here')
    idx_largest_cluster = np.argmax([len(c) for c in cluster_idx_list])
    #print('idx_largest_cluster:')
    #print(idx_largest_cluster)
    #input('Pause here')
    out_cluster = [last_output_list[i] for i in cluster_idx_list[idx_largest_cluster]]
    #print('out_cluster:')
    #print(out_cluster)
    #input('Pause here')

    #expected_output = np.mean(acqopt.get_last_output_list(), 0)
    expected_output = np.mean(out_cluster, 0)
    plt.plot(expected_output[0], expected_output[1], 'rs')
